    ...
    <section id="how-it-works" className="scroll-mt-16 py-16 sm:py-20 lg:py-28">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-2 text-3xl font-bold tracking-tight text-light-surface sm:text-4xl">
            {t("howItWorks.title")}
          </h2>

          <p className="mt-4 text-light-muted">{t("howItWorks.description")}</p>
        </div>
        ...
        <div className="mt-12 grid gap-8 md:grid-cols-3">
          {steps.map(({ icon: Icon, title, description }, index) => (
            <div
              key={title}
              className="relative text-center rounded-xl bg-light-surface border border-light-border p-6 shadow-sm sm:p-8 hover:border-brand-primary hover:text-brand-primary transition duration-200"
            >
              ...
              <p className="mt-3 leading-7 text-light-muted">{description}</p>
            </div>
          ))}
        </div>
      </div>
